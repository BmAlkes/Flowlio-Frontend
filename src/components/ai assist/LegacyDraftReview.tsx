...
export function LegacyDraftReview({draft,onCancel,onApply}:{draft:LegacyDraft;onCancel:()=>void;onApply:(draft:LegacyDraft)=>Promise<void>}){
 const {t}=useTranslation();const [data,setData]=useState(draft.data),[confirm,setConfirm]=useState(false),[busy,setBusy]=useState(false),[failed,setFailed]=useState(false);
 const validDates=!(data.startDate&&data.endDate&&data.startDate>data.endDate);
 const valid=validDates&&(draft.type==='client'?Boolean(data.name?.trim()&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email??'')&&String(data.password??'').length>=8):draft.type==='project'?Boolean(data.name?.trim()):Boolean(data.title?.trim()&&data.projectId));
 ...
 return <Dialog open onOpenChange={open=>{if(!open&&!busy)onCancel();}}><DialogContent className="max-h-[85dvh] overflow-y-auto"><DialogHeader><DialogTitle>{t('agent.review')}</DialogTitle><DialogDescription>{t('agent.legacyDescription')}</DialogDescription></DialogHeader>
  ...
  {draft.type==='client'&&<p className="text-xs text-muted-foreground">{t('hub.passwordNote')}</p>}
  {failed&&<p role="alert" className="text-sm text-destructive">{t('hub.saveError')}</p>}
  <div className="flex gap-2 justify-end"><Button variant="outline" disabled={busy} onClick={onCancel}>{t('agent.cancel')}</Button><Button disabled={busy||!confirm||!valid} onClick={async()=>{setBusy(true);setFailed(false);try{await onApply({type:draft.type,data});onCancel();}catch{setFailed(true);}finally{setBusy(false);}}}>{t('agent.apply',{count:1})}</Button></div>
