      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {fields.map((field) => (
            ...
            >
              <GripVertical className="h-4 w-4 text-muted-foreground/90 shrink-0" />

              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-foreground">{field.name}</span>
                  {field.required && (
                    <span className="text-xs font-semibold text-rose-600 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-500/30 px-1.5 py-0.5 rounded-full">
                      Required
                    </span>
                  )}
                </div>
