...
it("absence reductions never overwrite recurring weekly availability", async () => { render(<QueryClientProvider client={new QueryClient()}><MemoryRouter><CapacityMemberRow member={{ ...member, weeklyMinutes: 2400, availableMinutes: 1200, absenceMinutes: 1200 }} /></MemoryRouter></QueryClientProvider>); const user = userEvent.setup(); await user.click(screen.getByText(messages.configure)); expect(screen.getByLabelText(messages.weeklyHours)).toHaveValue(40); await user.click(screen.getByRole("button", { name: "common.save" })); await waitFor(() => expect(api.put).toHaveBeenCalledWith("/capacity/member", { weeklyMinutes: 2400, team: "Design" })); });
