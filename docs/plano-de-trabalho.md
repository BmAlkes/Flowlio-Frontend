...
| T22 | Automações entre módulos — `feat/t22-workflow-automations` | FE + BE | Gatilhos/condições/ações sobre eventos persistidos, simulação, histórico, limites de encadeamento e idempotência. | T10, T18, T20 | Backend 9594bdc publicado; frontend 6d83eaf em publicacao |
| T23 | Onboarding e ativação — `feat/t23-core-onboarding` | FE + BE | Checklist por papel levando ao primeiro cliente/projeto/entrega; persistência e métricas de ativação, sem completar etapas ficticiamente. | T18 | Implementada e validada; publicacao em andamento |
