...
| T04 | Sessão e organização ativa — `fix/t04-session-organization` | FE + BE | Unificar cliente auth; impedir fallback para vínculo inativo; revisar fluxo de cliente e revogação; cache privado isolado por identidade/organização; testar login, logout, OTP e 2FA. | T03 | Publicada; revisao automatizada de login e 2FA concluida |
...
| T11 | Conciliação de assinaturas — `fix/t11-subscription-reconciliation` | BE | Eventos persistidos/deduplicados, identificação indexada do provedor, transições consistentes, falha recuperável e testes de sandbox; revisar renovação e calendário. | T09, T10 | Adiada por decisao do usuario: sem PayPal Sandbox; nao publicar a branch |
...
| T16 | Localização e acessibilidade — `feat/t16-localization-accessibility` | FE + BE | Moeda e fuso explícitos, formatação consistente, traduções completas nos fluxos core, teclado/foco, contraste, RTL e limites de data testados. | T12 | Publicada no escopo documentado; Railway 0044645 e Cloudflare c61cc9e confirmados |
| T17 | CI e testes dos fluxos críticos — `test/t17-core-journeys` | FE + BE | Pipeline obrigatório de lint/test/build, banco de teste e contratos; jornadas de autenticação, autorização, execução e cobrança; nunca usar dados reais. | T03–T12 | Publicada; CI obrigatorio e protecao da main ativos nos dois repositorios |
| T18 | Proposta para projeto — `feat/t18-proposal-to-project` | FE + BE | Aprovação prepara projeto por template com tarefas/marcos/orçamento; revisão, rastreabilidade e proteção contra criação duplicada. | T03, T10, T12 | Publicada; Railway 220461d e Cloudflare 13a48f2 confirmados |
| T19 | Rentabilidade — `feat/t19-profitability` | FE + BE | Relacionar receitas, despesas, custo das horas e saldo não faturado; distinguir custo de tarifa de venda; moeda/competência consistentes e fórmulas testadas. | T07, T11, T16 | Publicada; Railway 1427e54 e Cloudflare 5e92f5d confirmados |
| T20 | Aprovação de entregas — `feat/t20-delivery-approval` | FE + BE | Cliente aprova/rejeita versão ou marco específico, histórico e pedido de ajuste; política de acesso e prevenção de aprovação de versão desatualizada. | T03, T12 | Publicada; BE 9215268 e FE 98f4d25 (inclui correcao T19) |
| T21 | Capacidade da equipe — `feat/t21-team-capacity` | FE + BE | Disponibilidade semanal, estimativas, dependências, sobrecarga e filtros por equipe; estados sem estimativa explícitos. | T08, T14, T16 | Publicada; Railway 426905e e Cloudflare 68b4420 confirmados |
| T22 | Automações entre módulos — `feat/t22-workflow-automations` | FE + BE | Gatilhos/condições/ações sobre eventos persistidos, simulação, histórico, limites de encadeamento e idempotência. | T10, T18, T20 | Publicada; Railway 9594bdc e Cloudflare 6d83eaf confirmados |
| T23 | Onboarding e ativação — `feat/t23-core-onboarding` | FE + BE | Checklist por papel levando ao primeiro cliente/projeto/entrega; persistência e métricas de ativação, sem completar etapas ficticiamente. | T18 | Publicada; Railway 4961341 e Cloudflare 272ca4e confirmados |
| T24 | Documentação operacional — `docs/t24-domain-runbooks` | FE + BE | README real, instalação, papéis, contratos, faturamento, jobs, releases e recuperação documentados e conferidos. | Atualizada ao longo das tarefas | Publicada; Railway eb7d40a e Cloudflare ec9f958 confirmados |
...
## T13 - Modularizacao incremental

- Branch `refactor/t13-domain-modules` nos dois repositorios, a partir da T12 publicada (BE `b1a51a1`, FE `45a5195`). T11 permanece separada; nenhuma conciliacao de assinatura ou migracao SQL incluida.
- Login: formulario separado dos casos de conclusao do login e tratamento de erros do provedor, em `src/features/auth`. Os dois callbacks foram extraidos preservando a logica existente, incluindo 2FA, refresh antes do dashboard, contas pendentes, demo e bloqueios. A compatibilidade legada de erros permanece isolada para futura simplificacao com cobertura.
- Projetos: listagem e consulta por ID sairam do controller para `src/modules/projects/read-projects.ts`. Filtros de autorizacao, joins, ordenacao, campos, valores padrao e restricao de acesso ao orcamento preservados.
- Pagamentos: atualizacao manual de status do link extraida para `src/modules/payments/update-payment-link-status.ts`. ID e organizacao continuam na mesma condicao de gravacao; validacao HTTP e respostas permanecem no controller. Nao ha novas chamadas de cobranca ou captura.
- Automacoes: 11 controllers utilizam um adaptador HTTP comum e o caso de uso `run-manual-automation`. Opcoes especificas (incluindo forceRun), mensagens, ordem de execucao/historico e tratamento de falhas preservados. Worker e agendamento nao foram alterados.
- Testes novos: oito de login e 17 de modulos backend; cobrem 2FA antes de consultar perfil, redirecionamentos, suspensao, erros legados, escopo de consulta/gravacao e execucao sem repeticao. Banco e provedores externos simulados, sem operacoes em dados reais.
- Onde conferir: `/auth/signin`, `/dashboard/project` e detalhe do projeto, `/dashboard/payment-links`, `/dashboard/settings/automations`. Refatoracao interna: o visual e os fluxos devem continuar iguais.

- Validacao: 147 testes backend aprovados; seis suites PostgreSQL nao executadas nesta rodada. Frontend com 122 casos: dois afetados por timeout no arquivo 2FA passaram na repeticao isolada dos tres testes desse arquivo (limite de 15s, execucao em 4,54s). Os oito testes novos de login passaram. Lint dos arquivos alterados e builds frontend/backend aprovados; contratos T12 continuam sincronizados. Backend `a7ce0b9` integrado na main e enviado ao Railway.

## T14 - Cache, paginacao e consultas

- Branch `perf/t14-data-access` em FE/BE. Projetos, tarefas e historico de horas aceitam page/pageSize, limite maximo 200, validacao 400 e ordenacao estavel por data/ID. Filtros de nome/numero/status de projeto e titulo de tarefa executados no banco, combinados com as politicas de acesso.
- Compatibilidade: chamadas antigas sem paginacao conservam a resposta integral durante o deploy. Calendarios, seletores e tabelas existentes precisam da colecao completa; seus hooks percorrem paginas de 100 registros, deduplicam IDs e rejeitam falhas intermediarias sem exibir sucesso parcial. Isso limita cada resposta HTTP, mas nao reduz o volume total dessas telas nem substitui seus filtros locais.
- Chaves de projetos/tarefas/horas incluem identidade, sessao, organizacao e papel. Invalida??es por dominio existentes continuam funcionando. Projetos e horas compartilham resultados frescos por 30 segundos; removido timestamp artificial dos GETs. API envia Cache-Control private/no-store. Polling do timer continua pausado com aba oculta, 5s ativo/30s inativo, com invalidacao entre abas da T08.
- Medicao real em PostgreSQL local, 1.100 projetos ficticios/1.000 autorizados: consulta integral 1 query, 19,59ms e 473.180 bytes; primeira pagina 1 query, 6,53ms e 47.325 bytes. Nao e medicao de producao nem da soma de todas as paginas. Teste percorreu as 10 paginas sem repeticao e validou filtros/escopo, tarefas e horas.
- Validacao: 153 testes backend aprovados, incluindo tres PostgreSQL da T14; seis suites de outros dominios sem banco nesta rodada. Frontend: 126 testes aprovados. T11 adiada por solicitacao do usuario, sem Sandbox; branch nao integrada.
- Onde conferir: Projects, Task Management, Time Tracking e os calendarios/seletores que usam essas listas. A alteracao principal e no acesso a dados; sem redesenho das telas.

## Fechamento da revisao T04

- Branch `fix/t04-login-recovery`, frontend. Falha/ausencia do perfil ou do refresh de contexto mantem o formulario disponivel com erro; nao anuncia sucesso nem inventa desafio OTP ou redirecionamento ao dashboard. 2FA continua usando o desafio explicito do provedor. Sucesso so e anunciado apos atualizar o contexto.
- Revisao automatizada inclui login ativo/pendente/demo, suspensao, OTP com sessao correspondente, ativacao/desativacao de 2FA, revogacao e descarte de respostas de outra identidade/organizacao. Testes nao substituem entrega real de e-mail; a ativacao real de 2FA ja havia sido confirmada pelo usuario.
- Onde conferir: `/auth/signin`; apos falha de rede/perfil, o formulario mostra erro e permite nova tentativa.

- Validacao do fechamento T04: 30 testes frontend e 32 backend aprovados, sem testes ignorados no recorte de autenticacao. Lint aprovado. T14 publicada: backend `8d82811`, frontend `7d11592`, Railway e Cloudflare confirmaram sucesso.
...
## T17 - CI e jornadas criticas

Configuracao e reproducao em [t17-core-journeys.md](./t17-core-journeys.md). Validar protecao da main e confirmar checks remotos antes do merge.
