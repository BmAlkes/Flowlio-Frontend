...
| T11 | Conciliação de assinaturas — `fix/t11-subscription-reconciliation` | FE + BE | Eventos persistidos/deduplicados, identificação indexada do provedor, transições consistentes, falha recuperável e testes de sandbox; revisar renovação e calendário. | T09, T10 | Implementada; validação sandbox pendente |
...
## T11 — Conciliação de assinaturas

- Branch `fix/t11-subscription-reconciliation` nos dois repositórios. Implementação e validação local prontas; merge e deploy aguardam a validação real no sandbox, prevista no critério desta etapa. O ambiente local não possui credenciais PayPal configuradas.
- Webhooks verificados são persistidos e deduplicados antes da confirmação de recebimento. A fila consulta o estado atual do PayPal e atualiza assinatura, organização e evento na mesma transação. Falhas têm retry; conciliação horária recupera alterações perdidas, inclusive de assinaturas em atraso.
- Identificador PayPal com índice único e migração dos dados antigos. Cancelamentos já solicitados são preservados. Duplicatas de identificador impedem a migração, sem atribuir uma assinatura à conta errada.
- Cancelamento confirma a interrupção no provedor e preserva o acesso até o fim do período pago. A intenção fica persistida para recuperação após falhas. Renovação usa datas do PayPal e confirmação de pagamento; planos gratuitos usam meses/anos do calendário UTC, incluindo finais de mês e anos bissextos.
- Ativação confere proprietário e plano, é transacional e não altera datas quando repetida. No checkout, uma aprovação pendente pode ser confirmada novamente usando a mesma assinatura, inclusive após recarregar a aba.
- Builds de frontend e backend aprovados. Frontend: 112 testes aprovados. Backend: suíte geral com 145 testes aprovados; após ampliar os casos de migração e rollback, os 22 testes PostgreSQL da T11 passaram. Há ainda seis testes de regras de calendário/pagamento incluídos na suíte geral. Provedor simulado nos testes locais; isso não substitui o sandbox real.
- Onde conferir após publicação: `/checkout`, na confirmação de uma aprovação pendente; `/dashboard/subscription`, no cancelamento, status e datas da assinatura. Recuperação e deduplicação acontecem no backend, sem nova tela. Operação documentada em `Flowlio-Backend/docs/subscription-reconciliation.md`.
