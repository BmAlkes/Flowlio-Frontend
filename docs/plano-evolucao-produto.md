...
Em 23/09/2026, o usuário autorizou continuar todos os processos faltantes, executando e validando as etapas pela sequência de dependências. T25, T26A/B, T27A/B, T28A/B, T29A/B, T30A/B, T31A/B, T32A/B, T33A/B e os refinamentos de clientes/leads estão publicados. T33A/B foi ampliada para agente contextual nas áreas internas, conforme pedido do usuário. A T11 continua separada, dependente de validação PayPal Sandbox.
...
| 10 | T33A/B — IA contextual e revisável | FE + BE | T05, T19, T26A; T28 para comparação de escopo | Grande | Publicada |
...
## T33 — Agente interno contextual em todo o Flowlio
...
**Aceite:** nenhuma informação entre organizações/clientes; citações apontam a registros reais autorizados; ausência de dados é explícita; conteúdo de documentos é dado, não instrução para executar ações; consumo respeita T05; retry não duplica chamadas ao provedor. Revisão por padrão, com autonomia opcional por solicitação limitada à criação e ao planejamento de tarefas internas. Compartilhamento com cliente exige confirmação separada. Clientes não acessam geração, contexto ou histórico, mas podem receber e responder aos briefings publicados pela equipe sem consumir tokens. Avaliado com dados fictícios e provedor simulado; qualidade factual ao vivo não foi avaliada.

**Onde conferir:** botão **Flowlio AI** na barra superior interna e atalhos em clientes, leads e tarefas. Fontes, histórico, revisão editável e recibos; ações para tarefas, follow-up e briefing. Ferramentas existentes de propostas, imagens, arquivos e cadastros preservadas e aprimoradas. Branch frontend/backend: `feat/t33-contextual-agent`. Contrato, cobertura por área, validação e limites em [Agente interno contextual](t33-internal-ai-agent.md). A presença global não implica execução de todas as operações; rotinas arbitrárias recorrentes por linguagem natural ficam fora deste incremento.
...
- T33A/B: publicada em 26/09/2026, entrega inicial FE `21de5b9`, BE `7ebee6c`; CI branch/main, Railway, saúde da API, Cloudflare e SHA no artefato servido confirmados. Agente interno global, fontes, revisão editável, histórico e execução limitada. Clientes recebem briefings revisados pelo portal, sem acesso à geração. Evidências, cobertura e limites em [Agente interno contextual](t33-internal-ai-agent.md).
